...
async function initDB() {
  ...
  try {
    ...
    if (!process.env.DATABASE_URL) {
      ...
      console.log('Using database defined in DATABASE_URL.');
    }
...
async function initPgDB(connectionString) {
  ...
  try {
    ...
    // Drop tables to apply clean new constraints
    console.log('Dropping existing tables for schema synchronization...');
    await client.query('DROP TABLE IF EXISTS list_contacts CASCADE;');
    await client.query('DROP TABLE IF EXISTS contact_lists CASCADE;');
    await client.query('DROP TABLE IF EXISTS contacts CASCADE;');
    await client.query('DROP TABLE IF EXISTS smtp_configs CASCADE;');
    await client.query('DROP TABLE IF EXISTS followup_sequences CASCADE;');
    ...
    await client.query(`
      CREATE TABLE recipients (
        ...
        reason VARCHAR(255) DEFAULT '',
        sentAt VARCHAR(50) DEFAULT '',
        openedAt TIMESTAMP DEFAULT NULL,
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_recipients_campaignId ON recipients(campaignId);');
    ...
    // 4. Create Follow-up Sequences Table
    await client.query(`
      CREATE TABLE followup_sequences (
        id SERIAL PRIMARY KEY,
        campaignId VARCHAR(50) NOT NULL,
        ...
        executedAt TIMESTAMP DEFAULT NULL,
        sentCount INT DEFAULT 0,
        FOREIGN KEY (campaignId) REFERENCES campaigns(id) ON DELETE CASCADE
      );
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_fu_campaignId ON followup_sequences(campaignId);');
    await client.query('CREATE INDEX IF NOT EXISTS idx_fu_status ON followup_sequences(status);');
    ...
    `);
    await client.query('CREATE INDEX IF NOT EXISTS idx_audit_campaignId ON audit_logs(campaignId);');
    ...
    // 8. Create List Contacts Junction Table
    await client.query(`
      CREATE TABLE list_contacts (
        list_id VARCHAR(50),
        contact_id VARCHAR(50),
    ...
    `, ['default', 'Default SMTP Server', 'smtp.sendgrid.net', 587, 'apikey', 'SG.mock_api_key_value_example', 'TLS', 'campaigns@enterprise.com', 'Enterprise Bulk Mailer', 1]);
    console.log('Default SMTP Config seeded.');
    ...
    // Seed default admin user (Vaibhav Soni)
    await client.query(`
      INSERT INTO users (id, name, email, password, role, status, avatar)
      VALUES ($1, $2, $3, $4, $5, $6, $7);
    `, ['1', 'Vaibhav Soni', 'vaibhavsoni1059@gmail.com', hashPassword('admin123'), 'Admin', 'Active', '/male_boy_avatar.png']);
    console.log('Seeded administrator user.');

    // 11. Create Templates Table
    await client.query(`
      CREATE TABLE IF NOT EXISTS templates (
        id VARCHAR(50) PRIMARY KEY,
        name VARCHAR(255),
    ...
    // Seed default templates if table is empty
    const templateRows = await client.query('SELECT COUNT(*) as count FROM templates;');
    if (parseInt(templateRows.rows[0].count) === 0) {
      const defaultTemplates = [
        {
          id: 't1',
          name: 'Monthly Product Newsletter',
