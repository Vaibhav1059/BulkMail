...
export const CreateCampaign = () => {
  const { campaignWorkspace, setCampaignWorkspace, saveCampaign, sendTestEmail, logEvent, templates, csvData } = useContext(AppContext);
  const navigate = useNavigate();

  const variablesToDisplay = csvData?.headers?.length > 0 
    ? [...new Set(['name', 'email', 'company', ...csvData.headers])].map(h => `{{${h}}}`)
    : ['{{name}}', '{{email}}', '{{company}}'];
